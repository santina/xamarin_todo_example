	...
	public class TodoDatabase 
	{
		...
		public TodoDatabase (string dbPath) 
		{
			...
			if (!exists) {
				...
				connection.Open ();
				var commands = new[] {
					"CREATE TABLE [Items] (_id INTEGER PRIMARY KEY ASC, Name NTEXT, Notes NTEXT, Email NTEXT, Done INTEGER);"
				};
		...
		public IEnumerable<TodoItem> GetItems ()
		{
			...
			lock (locker) {
				...
				using (var contents = connection.CreateCommand ()) {
					contents.CommandText = "SELECT [_id], [Name], [Notes], [Email], [Done] from [Items]";
		...
		public TodoItem GetItem (int id) 
		{
			...
			lock (locker) {
				...
				using (var command = connection.CreateCommand ()) {
					command.CommandText = "SELECT [_id], [Name], [Notes], [Email], [Done] from [Items] WHERE [_id] = ?";
		...
		public int SaveItem (TodoItem item) 
		{
			...
			lock (locker) {
				if (item.ID != 0) {
					...
					connection.Open ();
					using (var command = connection.CreateCommand ()) {
						command.CommandText = "UPDATE [Items] SET [Name] = ?, [Notes] = ?, [Email] = ?, [Done] = ? WHERE [_id] = ?;";
					...
					connection.Open ();
					using (var command = connection.CreateCommand ()) {
						command.CommandText = "INSERT INTO [Items] ([Name], [Notes], [Email], [Done]) VALUES (? ,?, ?)";
						command.Parameters.Add (new SqliteParameter (DbType.String) { Value = item.Name });
						command.Parameters.Add (new SqliteParameter (DbType.String) { Value = item.Notes });
                        command.Parameters.Add (new SqliteParameter(DbType.String) { Value = item.Email });
                        command.Parameters.Add (new SqliteParameter (DbType.Int32) { Value = item.Done });
